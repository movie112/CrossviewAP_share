...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", default="C:\\Users\\movie\\Documents\\workspace\\flitto\\Multilingual-AWE_yh0311\\multiview-babel-phone\\expt\\sample\\config.json", help="configuration filename")
    ...
    def get_xsampa_to_ipa(feature_fn): # xsampa_phonetic_features.json
    ...
    xsampa_to_ipa = get_xsampa_to_ipa(config.feature_fn) # 음소(phonetic)별 정보
    ...
    for lang_id in range(n_lang):
        ...
        with torch.no_grad():
            ...
            for batch in datasets[lang_id].loader:
                if batch is None:
                    log.warning(f"[lang_id={lang_id}] Empty batch encountered. Skipping.")
                    continue
                ids = batch.pop("ids")
                inv = batch.pop("inv")
        ...
        # write csv
        with open(f"{expt_dir}/{this_lang}-acoustic-labels.tsv", 'w', encoding='utf-8') as f:
            for word in words1:
                f.write(f"{word}\n")
        ...
        with open(f"{expt_dir}/{this_lang}-word-labels.tsv", 'w', encoding='utf-8') as f:
            for word in words2:
                f.write(f"{word}\n")
